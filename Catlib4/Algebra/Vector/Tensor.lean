...
structure BilinearMap {K : Field} (U V W : KVect K) where
  f : U → V → W
...
instance {K : Field} (U V W : KVect K) :
  CoeFun (BilinearMap U V W) (fun _ => U → V → W) where
  coe f := f.f

section

variable {K : Field} {U V W : KVect K} (f : BilinearMap U V W)
...
theorem BilinearMap.eq {K : Field} {U V W : KVect K} :
  ∀ {f g : BilinearMap U V W}, f.f = g.f → f = g
  | ⟨ _, _, _, _, _ ⟩, ⟨ _, _, _, _, _ ⟩, rfl => rfl

theorem BilinearMap.ext {K : Field} {U V W : KVect K}
  {f g : BilinearMap U V W} (h : ∀ x y, f x y = g x y) : f = g :=
  BilinearMap.eq <| funext λ x => funext λ y => h x y

def BilinearMap.compose {K : Field} {U V W E : KVect K}
  (f : W ⟶ E) (g : BilinearMap U V W) : BilinearMap U V E where
  f u v := f (g u v)
  map_smul_l' := by simp
  map_smul_r' := by simp
  map_add_l' := by simp
...
@[simp] theorem BilinearMap.compose_val {K : Field} {U V W E : KVect K}
  (f : W ⟶ E) (g : BilinearMap U V W)
  (u : U) (v : V) : BilinearMap.compose f g u v = f (g u v) := rfl

def vanish_map_smul_l {K : Field} {U V : KVect K} (u : U) (v : V) :
  linearized K K ⟶ linearized K (U × V) :=
  linearizeF λ μ => μ • l[(u, v)] + -l[(μ • u, v)]

def vanish_map_smul_r {K : Field} {U V : KVect K} (u : U) (v : V) :
  linearized K K ⟶ linearized K (U × V) :=
  linearizeF λ μ => μ • l[(u, v)] + -l[(u, μ • v)]

def vanish_map_add_l {K : Field} (U : KVect K) {V : KVect K} (w : V) :
  linearized K (U × U) ⟶ linearized K (U × V) :=
  linearizeF λ (u, v) => l[(u + v, w)] + -l[(u, w)] + -l[(v, w)]

def vanish_map_add_r {K : Field} {U : KVect K} (u : U) (V : KVect K) :
  linearized K (V × V) ⟶ linearized K (U × V) :=
  linearizeF λ (v, w) => l[(u, v + w)] + -l[(u, v)] + -l[(u, w)]

def bilinear_as_from_linearized {K : Field} {U V W : KVect K}
  (f : BilinearMap U V W) : linearized K (U × V) ⟶ W where
  f := linearizeF λ (u, v) => f u v
  map_smul := by simp
  map_add := by simp

theorem factor_vanish_map_smul_l {K : Field} {U V W : KVect K}
  {u : U} {v : V} {f : BilinearMap U V W} :
  vanish_map_smul_l u v ≫ bilinear_as_from_linearized f = 0 := by
  apply factor_of_vanish
  simp [bilinear_as_from_linearized]

theorem factor_vanish_map_smul_r {K : Field} {U V W : KVect K}
  {u : U} {v : V} {f : BilinearMap U V W} :
  vanish_map_smul_r u v ≫ bilinear_as_from_linearized f = 0 := by
  apply factor_of_vanish
  simp [bilinear_as_from_linearized]

theorem factor_vanish_map_add_l {K : Field} {U V W : KVect K}
  {w : V} {f : BilinearMap U V W} :
  vanish_map_add_l U w ≫ bilinear_as_from_linearized f = 0 := by
  apply factor_of_vanish
  intro ⟨ u, v ⟩
...
theorem factor_vanish_map_add_r {K : Field} {U V W : KVect K}
  {u : U} {f : BilinearMap U V W} :
  vanish_map_add_r u V ≫ bilinear_as_from_linearized f = 0 := by
  apply factor_of_vanish
  intro ⟨ v, w ⟩
...
inductive ProdSupport {K : Field} (U V : KVect K) where
  | smul_l (u : U) (v : V)
  | smul_r (u : U) (v : V)
  | add_l (w : V)
  | add_r (u : U)

def diagram_space {K : Field} (U V : KVect K) :
  ProdSupport U V → KVect K
  | .smul_l _ _ => linearized K K
  | .smul_r _ _ => linearized K K
  | .add_l _ => linearized K (U × U)
  | .add_r _ => linearized K (V × V)

def diagram_map {K : Field} (U V : KVect K) :
  KVect.product (diagram_space U V) ⟶ linearized K (U × V) :=
  KVect.map_product λ x => match x with
  | .smul_l u v => vanish_map_smul_l u v
  | .smul_r u v => vanish_map_smul_r u v
  | .add_l w => vanish_map_add_l U w
  | .add_r u => vanish_map_add_r u V

theorem factor_vanish_diagram {K : Field} {U V W : KVect K} (f : BilinearMap U V W) :
  diagram_map U V ≫ (bilinear_as_from_linearized f) = 0 := by
...
def tensor_space {K : Field} (U V : KVect K) : KVect K :=
  cokernel (diagram_map U V)

def projectorF {K : Field} (U V : KVect K) :=
  cokernel_projector (diagram_map U V)

def from_linearized_as_bilinear {K : Field} {U V W : KVect K}
  (f : linearized K (U × V) ⟶ W)
  (h : diagram_map U V ≫ f = 0)
...
theorem bilinear_as_from_linearized_eq {K : Field} {U V W : KVect K}
...
theorem bilinear_as_from_linearized_map {K : Field} {U V W E : KVect K}
  (f : W ⟶ E) (g : BilinearMap U V W) :
  bilinear_as_from_linearized (BilinearMap.compose f g) =
    bilinear_as_from_linearized g ≫ f := by
  apply LinearMap.ext
  intro x
...
theorem linearize_bilinearize {K : Field} {U V W : KVect K}
  (f : linearized K (U × V) ⟶ W)
  (h : diagram_map U V ≫ f = 0) :
...
def tensor_factor {K : Field} {U V W : KVect K} (f : BilinearMap U V W) :
  tensor_space U V ⟶ W :=
  cokernel_factor (diagram_map U V)
    (bilinear_as_from_linearized f)
    (factor_vanish_diagram f)

def tensorα {K : Field} {U V : KVect K} : BilinearMap U V (tensor_space U V) :=
  from_linearized_as_bilinear (cokernel_projector <| diagram_map U V) cokernel_vanish

theorem tensor_factor_sound {K : Field} {U V W : KVect K} (f : BilinearMap U V W) :
...
@[simp] theorem tensor_factor_val {K : Field} {U V W : KVect K} (f : BilinearMap U V W) :
  ∀ {x y}, tensor_factor f (tensorα x y) = f x y := by
  intro x y
  conv => rhs; rw [tensor_factor_sound f]

theorem map_tensor_ext {K : Field} {U V W : KVect K}
  {f g : tensor_space U V ⟶ W}
...
theorem TensorMap.ext {K : Field} {U V W : KVect K}
  {f g : tensor_space U V ⟶ W}
