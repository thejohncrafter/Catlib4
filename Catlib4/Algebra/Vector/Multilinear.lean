...
def bilinear_of_map_linear_right {K : Field} (U V W : KVect K)
  (f : U → (V ⟶ W))
...
@[simp] theorem bilinear_of_map_linear_right_val {K : Field} (U V W : KVect K)
  (f : U → (V ⟶ W))
  (map_smul : ∀ x : V, ∀ μ : K, ∀ v : U, f (μ • v) x = μ • f v x)
  (map_add : ∀ x : V, ∀ v w : U, f (v + w) x = f v x + f w x)
  (u : U) (x : V)
...
def bilinear_of_map_linear_left {K : Field} (U V W : KVect K)
  (f : V → (U ⟶ W))
...
@[simp] theorem bilinear_of_map_linear_left_val {K : Field} (U V W : KVect K)
  (f : V → (U ⟶ W))
  (map_smul : ∀ x : U, ∀ μ : K, ∀ v : V, f (μ • v) x = μ • f v x)
  (map_add : ∀ x : U, ∀ v w : V, f (v + w) x = f v x + f w x)
  (u : U) (x : V)
...
def linear_left_of_bilinear {K : Field} {U V W : KVect K}
  (f : BilinearMap U V W) (u : U) : V ⟶ W where
  f := λ v => f u v
  map_smul := by simp
  map_add := by simp

@[simp] theorem linear_left_of_bilinear_val
  {K : Field} {U V W : KVect K} (f : BilinearMap U V W)
  : ∀ u v, linear_left_of_bilinear f u v = f u v
  := λ _ _ => rfl

def linear_right_of_bilinear {K : Field} {U V W : KVect K}
  (f : BilinearMap U V W) (v : V) : U ⟶ W where
  f u := f u v
  map_smul := by simp
  map_add := by simp
...
structure Linear {K : Field} {V W : KVect K} (f : V → W) where
...
def linear_leaf {K : Field} {U V : KVect K} (f : U → V) (h : Multilinear V (.leaf U) f)
  : U ⟶ V where
  f := f
...
theorem prod_tensor_eq {K : Field} {W : KVect K} (l : BinaryTree (KVect K))
  (f g : prod_tensor l ⟶ W)
...
def build_step_left {K : Field} {W V : KVect K} {l : BinaryTree (KVect K)}
  (f : prod_set (.node_left l V) → W)
  (map_smul : ∀ x : prod_set l, ∀ μ : K, ∀ v : V, f (x, μ • v) = μ • f (x, v))
  (map_add : ∀ x : prod_set l, ∀ v w : V, f (x, v + w) = f (x, v) + f (x, w))
  (recursive : (v : V) → { lin : prod_tensor l ⟶ W // ∀ x, lin (prod_tensorα x) = f (x, v) })
  : { lin : prod_tensor (.node_left l V) ⟶ W // ∀ x, lin (prod_tensorα x) = f x } :=
  ⟨ by
    apply tensor_factor
...
def build_step_right {K : Field} {W U : KVect K} {r : BinaryTree (KVect K)}
  (f : prod_set (.node_right U r) → W)
  (map_smul : ∀ x : prod_set r, ∀ μ : K, ∀ v : U, f (μ • v, x) = μ • f (v, x))
  (map_add : ∀ x : prod_set r, ∀ v w : U, f (v + w, x) = f (v, x) + f (w, x))
  (recursive : (u : U) → { l : prod_tensor r ⟶ W // ∀ x, l (prod_tensorα x) = f (u, x) })
  : { l : prod_tensor (.node_right U r) ⟶ W // ∀ x, l (prod_tensorα x) = f x } :=
  ⟨ by
    apply tensor_factor
...
private opaque tensor_multifactor_ind {K : Field} {V : KVect K} (l : BinaryTree (KVect K))
  (f : prod_set l → V) (h : Multilinear V l f)
  : { l : prod_tensor l ⟶ V // ∀ x, l (prod_tensorα x) = f x } := match l with
  | .leaf U =>
    ⟨ linear_leaf f h
    , λ _ => rfl ⟩
...
def tensor_multifactor {K : Field} {V : KVect K} (l : BinaryTree (KVect K))
  (f : prod_set l → V) (h : Multilinear V l f) : prod_tensor l ⟶ V :=
  (tensor_multifactor_ind l f h).1

def tensor_multifactor_val {K : Field} {V : KVect K} (l : BinaryTree (KVect K))
  (f : prod_set l → V) (h : Multilinear V l f) :
